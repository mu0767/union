...
function fixture(reachedFinal){
  ...
  return {users,bosses,results,locks:[],settings:{startAt:'2026-09-07T05:00',damageTolerance:0}};
}

for(const reachedFinal of [true,false]){
  const state=fixture(reachedFinal);
  const plan=await solveRaidCpSat(state);
...
// Filling the allowed HP gap in an earlier round must not outrank last-round damage.
...
const gapPlan=await solveRaidCpSat(gap);
