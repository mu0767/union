...
레이드 플래너는 수학적 최적값을 끝까지 증명하는 것보다 실제 운영에 충분히 좋은 계획을 빠르게 찾는 **anytime 전역 최적화**를 사용한다.

- 전체 R1~최종보스를 한 모델에서 계산한다.
- 일반 보스는 잔여 HP 10억 이하를 계획상 클리어로 인정한다.
- 레벨별 공격력으로 보정한 속성별 상대 효율(중앙값 100%)을 이용해 잘 치는 파티를 초기 탐색 우선순위로 사용한다.
- 우선배정은 고정하지 않는다. 모든 후보를 남겨 니케 공유와 대체 가능성을 포함한 전역 교환을 허용한다.
- 최종보스 도달 시 최종보스 딜, 미도달 시 마지막 도달 라운드 유효 딜을 높인다.
- 작은 딜 차이의 최적성 증명보다 큰 구조적 개선과 과도한 일반 보스 낭비 제거를 우선한다.
- 기본 최대 60초 동안 여러 탐색을 병렬 실행하며, 시간이 늘수록 더 좋은 해를 찾을 수 있다.

세부 구현/회귀 기준은 `AGENTS.md`와 `PROJECT_STATUS.md`를 참고한다.
