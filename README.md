...
레이드 운영의 **최적 계획 계산 / 남은 계획 재계산**은 `planner-cpsat-engine.js`의 OR-Tools CP-SAT를 브라우저 Web Worker에서 실행합니다. Python 서버나 사용자 설치가 필요 없습니다. 목적함수는 **도달 라운드 → 최종보스 딜(미도달 시 마지막 도달 라운드의 유효 딜) → 오버딜 감소** 순서입니다. 목표 딜을 먼저 확보한 뒤에만 오버딜을 줄이며, 남은 공격권도 호환되는 편성을 확보해 추가 배치합니다. 제한 시간 내 최적성 증명을 마치지 못한 경우 실행 가능한 계획과 최적성이 증명된 계획을 구분합니다. 시간표는 현재 라운드별 표시용입니다.

`verify-planner-objectives.mjs`는 오버딜이 더 커도 최종보스/마지막 라운드 딜이 더 큰 계획을 선택하는지 검증합니다. `verify-cpsat-browser.mjs`는 실제 계산 버튼, 목표 딜 표시, 니케 중복, 공격권 사용, ±10억 오차 및 레벨 보정 퍼센트를 검증합니다.
