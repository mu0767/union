...
export async function solveRaidHybrid(state,progress=()=>{}){
  ...
  const partyAlternatives=new Map();
  for(const c of raw){const k=c.user.id+'|'+c.party.id;if(!partyAlternatives.has(k))partyAlternatives.set(k,new Set());partyAlternatives.get(k).add(c.boss.element);}
  const elementSupply=new Map(ELEMENTS.map(e=>[e,new Set(raw.filter(c=>c.boss.element===e).map(c=>c.user.id)).size]));
  const nikkeUse=new Map();
  for(const c of raw)for(const n of c.party.nikkes)add(nikkeUse,n,1);
  const rarityCost=c=>{
  ...
  const conflicts=(sel,c)=>{
    let count=0;const used=new Set();
    for(const x of sel)if(x.user.id===c.user.id){count++;for(const n of x.party.nikkes)used.add(n);}
    if(count>=c.user.attacksLeft)return true;
    return c.party.nikkes.some(n=>used.has(n));
  };
  const canAdd=(sel,c)=>!conflicts(sel,c);

  function bundleCandidates(boss,sel,limit=18){
    ...
    const out=[];
    const push=arr=>{
      let d=0;for(const c of arr)d+=c.damage;if(d<need)return;
      const over=d-need,es=arr.reduce((s,c)=>s+eff(c),0)/arr.length;
      const rare=arr.reduce((s,c)=>s+rarityCost(c),0);
      const ds=arr.map(c=>c.damage),spread=arr.length>1?(Math.max(...ds)-Math.min(...ds))/Math.max(...ds):0;
      const score=over/Math.max(1,need) + arr.length*0.025 + rare*0.04 - es*0.06 - spread*0.012;
      out.push({arr,score,over});
    };
    for(let i=0;i<pool.length;i++)push([pool[i]]);
    for(let i=0;i<pool.length;i++)for(let j=i+1;j<pool.length;j++){
      if(pool[i].user.id===pool[j].user.id&&pool[i].party.nikkes.some(n=>pool[j].party.nikkes.includes(n)))continue;
      push([pool[i],pool[j]]);
    }
    const top=pool.slice(0,9);
    for(let i=0;i<top.length;i++)for(let j=i+1;j<top.length;j++)for(let k=j+1;k<top.length;k++){
      const a=[top[i],top[j],top[k]];let ok=true;
      for(let x=0;x<a.length;x++)for(let y=x+1;y<a.length;y++)if(a[x].user.id===a[y].user.id&&a[x].party.nikkes.some(n=>a[y].party.nikkes.includes(n)))ok=false;
      if(ok)push(a);
    }
    out.sort((a,b)=>a.score-b.score||a.over-b.over);
    return out.slice(0,limit).map(x=>x.arr);
  }
  ...
  function stateScore(s){
    ...
    return waste/1e9 + s.sel.length*0.12 + rare*0.05 - effSum*0.08;
  }

  progress('초기 Beam Search · 실행 가능한 좋은 계획 생성 중…');
  ...
  // If the final is reachable, spend remaining resources on high final damage.
  if(reached===3){
  ...
  const better=(a,b)=>a.stage!==b.stage?a.stage>b.stage:a.target!==b.target?a.target>b.target:a.waste<b.waste;

  // Cheap deterministic 1:1 round swaps. This explicitly catches cases such as
  // R1=216 / R2=200 when swapping them keeps clears and improves later damage/waste.
  progress('로컬 개선 · 라운드 간 obvious swap 정리 중…');
  let improved=true,passes=0;
  while(improved&&passes++<6){
    improved=false;const base=evalPlan(heuristic);
    outer:for(let i=0;i<heuristic.length;i++)for(let j=i+1;j<heuristic.length;j++){
      const a=heuristic[i],b=heuristic[j];
      ...
      if(!aa||!bb)continue;
      const trial=[...heuristic];trial[i]=aa;trial[j]=bb;
      const ev=evalPlan(trial);
      if(better(ev,base)){heuristic=trial;improved=true;break outer;}
    }
  }

  // CP-SAT global improvement, seeded with the complete feasible heuristic plan.
  progress('CP-SAT 전역 개선 모델 구성 중…');
  const solver=await CpSolver.create();
  const model=new CpModel('union-raid-hybrid');
  const sum=xs=>xs.reduce((a,x)=>a.plus(x instanceof LinearExpr?x:x.toLinearExpr()),LinearExpr.fromConstant(0));
  const vars=[];for(const c of raw){const x=model.newBoolVar('x_'+c.id);vars.push({...c,x});}
  ...
  const chosenKeys=new Set(heuristic.map(keyOf));for(const c of vars)model.addHint(c.x,chosenKeys.has(keyOf(c))?1:0);for(let r=0;r<3;r++)model.addHint(clear[r],evalPlan(heuristic).stage>r?1:0);
  const solve=(label,obj,mode,seconds)=>{if(seconds<=0.05)return null;progress(label);mode==='max'?model.maximize(obj):model.minimize(obj);return solver.solve(model,{maxTimeInSeconds:Math.min(seconds,timeLeft()),numWorkers:globalThis.crossOriginIsolated?Math.min(8,Number(globalThis.navigator?.hardwareConcurrency)||1):1,randomSeed:Number(state.__solverSeed)||1,randomizeSearch:true});};
  const stageExpr=sum(clear);
  let result=solve('전역 개선 1/3 · 도달 라운드 확인 중…',stageExpr,'max',Math.min(3,timeLeft()));
  if(!result||!feasible(result.status))throw new Error('전역 개선에서 실행 가능한 해를 찾지 못했습니다.');
  ...
  const targetExpr=targetRound===4?sum(vars.filter(c=>c.boss.round===4).map(c=>c.x.times(c.damage))):sum(normal.filter(b=>b.round===targetRound).map(b=>effective.get(b.id)));
  ...
  const gran=Math.min(Math.max(1,tolerance||1),Math.max(1,Math.floor(targetMax/100)));const band=model.newIntVar(0,Math.max(0,Math.floor(targetMax/gran)),'band');model.add(targetExpr.ge(band.times(gran)));
  const cleanupReserve=Math.min(5,Math.max(2,timeLeft()*0.25));
  const r2=solve('전역 개선 2/3 · 큰 딜 개선 탐색 중…',band,'max',Math.max(0.5,timeLeft()-cleanupReserve));if(r2&&feasible(r2.status))result=r2;
  ...
  const wastes=[];for(const b of normal.filter(b=>b.round<targetRound)){const max=vars.filter(c=>c.boss.id===b.id).reduce((s,c)=>s+c.damage,0);if(!max)continue;const w=model.newIntVar(0,Math.max(0,max-required(b)),'w_'+b.id);model.add(w.ge(assigned.get(b.id).minus(required(b))));wastes.push(w);}
  if(wastes.length&&timeLeft()>0.2){const r3=solve('전역 개선 3/3 · 앞 라운드 낭비 정리 중…',sum(wastes),'min',timeLeft());if(r3&&feasible(r3.status))result=r3;}

  let selected=vars.filter(c=>result.value(c.x)>0.5).map(({x,...c})=>c);
  // Final deterministic sanity swap pass, so trivial beneficial swaps never depend on CP-SAT luck.
  improved=true;passes=0;while(improved&&passes++<8){improved=false;const base=evalPlan(selected);outer2:for(let i=0;i<selected.length;i++)for(let j=i+1;j<selected.length;j++){const a=selected[i],b=selected[j];if(a.boss.round===4||b.boss.round===4||a.boss.element!==b.boss.element||a.boss.round===b.boss.round)continue;const aa=raw.find(c=>c.user.id===a.user.id&&c.party.id===a.party.id&&c.boss.round===b.boss.round),bb=raw.find(c=>c.user.id===b.user.id&&c.party.id===b.party.id&&c.boss.round===a.boss.round);if(!aa||!bb)continue;const t=[...selected];t[i]=aa;t[j]=bb;const ev=evalPlan(t);if(better(ev,base)){selected=t;improved=true;break outer2;}}}
  ...
  const rem=new Map(bosses.map(b=>[b.id,b.round===4?'infinite':remaining(b)])),counts=new Map();const origin=new Date(state.settings?.startAt||Date.now());const pad=n=>String(n).padStart(2,'0');const iso=d=>`${d.getFullYear()}-${pad(d.getMonth()+1)}-${pad(d.getDate())}T${pad(d.getHours())}:${pad(d.getMinutes())}`;
