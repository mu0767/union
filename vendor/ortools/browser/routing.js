...
var nextTransitCallbackId = 1, routingModulePromise = null, routingModule = null;
...
var RoutingIndexManager = class {
  ...
  indexToNodeSync(index) {
    if (!this.module) {
      let node = this.indexToNodeMap[index];
  ...
  createStartsEndsManager(starts, ends) {
    ...
    let bytes = Int32Array.BYTES_PER_ELEMENT * this.numVehicles, startsPtr = this.module._malloc(bytes), endsPtr = this.module._malloc(bytes);
    try {
  ...
  createSyntheticIndexMapping() {
    ...
    let seenTerminals = /* @__PURE__ */ new Set(), terminalIndex = (node) => {
      if (!seenTerminals.has(node))
        return seenTerminals.add(node), node;
      let index = this.indexToNodeMap.length;
  ...
  RegisterTransitCallback(callback) {
    var _a;
    if (!this.module) {
      let evaluatorIndex2 = this.nextWorkerEvaluatorIndex++;
    ...
    let callbackId = nextTransitCallbackId++;
    ...
    let evaluatorIndex = this.module._routing_register_transit_callback(this.handle, callbackId);
    if (evaluatorIndex < 0)
  ...
  async solveWithWorkerRequest(parameters) {
    ...
    let assignment = new Assignment(this, response.result);
  ...
  async SolveWithParameters(parameters = DefaultRoutingSearchParameters()) {
    ...
    let assignment = new Assignment(this);
    return this.lastObjectiveValue = assignment.ObjectiveValue(), this.runAtSolutionCallbacks(), assignment;
  }
  async Solve() {
  ...
  RegisterTransitMatrix(matrix) {
    return this.RegisterTransitCallback((fromIndex, toIndex) => {
      let fromNode = this.manager.IndexToNode(fromIndex), toNode = this.manager.IndexToNode(toIndex);
  ...
  AddDimension(transitIndex, slackMax, capacity, fixStartCumulToZero, name) {
    ...
    let created = this.withCString(name, (namePtr) => this.moduleRef._routing_add_dimension(
      this.handle,
      transitIndex,
      toInt64(slackMax),
      toInt64(capacity),
  ...
  AddDimensionWithVehicleCapacity(transitIndex, slackMax, capacities, fixStartCumulToZero, name) {
    ...
    let capacityArray = toInt64Array(capacities), bytes = new Uint8Array(capacityArray.buffer, capacityArray.byteOffset, capacityArray.byteLength), ptr = this.module._malloc(bytes.byteLength);
    this.module.HEAPU8.set(bytes, ptr);
    try {
      let created = this.withCString(name, (namePtr) => this.moduleRef._routing_add_dimension_with_vehicle_capacity(
        this.handle,
        transitIndex,
        toInt64(slackMax),
        ptr,
  ...
  AddDimensionWithVehicleTransits(transitIndices, slackMax, capacity, fixStartCumulToZero, name) {
    if (!this.module) {
      let indices = Array.isArray(transitIndices) ? transitIndices : [transitIndices];
      return this.dimensionNames.add(name), this.operations.push({
    ...
    let evaluatorBytes = toInt32Bytes(transitIndices), ptr = this.module._malloc(evaluatorBytes.byteLength);
    this.module.HEAPU8.set(evaluatorBytes, ptr);
    try {
      let created = this.withCString(name, (namePtr) => this.moduleRef._routing_add_dimension_with_vehicle_transits(
        this.handle,
        ptr,
  ...
  AddConstantDimension(value, capacity, fixStartCumulToZero, name) {
    ...
    let evaluatorIndex = this.withCString(name, (namePtr) => this.moduleRef._routing_add_constant_dimension(
      this.handle,
      toInt64(value),
      toInt64(capacity),
      fixStartCumulToZero ? 1 : 0,
  ...
  AddVectorDimension(values, capacity, fixStartCumulToZero, name) {
    ...
    let valueArray = toInt64Array(values), bytes = new Uint8Array(valueArray.buffer, valueArray.byteOffset, valueArray.byteLength), ptr = this.module._malloc(bytes.byteLength);
    this.module.HEAPU8.set(bytes, ptr);
    try {
      let evaluatorIndex = this.withCString(name, (namePtr) => this.moduleRef._routing_add_vector_dimension(
        this.handle,
        ptr,
        valueArray.length,
        toInt64(capacity),
  ...
  AddMatrixDimension(matrix, capacity, fixStartCumulToZero, name) {
    ...
    let flat = matrix.flat(), valueArray = toInt64Array(flat), bytes = new Uint8Array(valueArray.buffer, valueArray.byteOffset, valueArray.byteLength), ptr = this.module._malloc(bytes.byteLength);
    this.module.HEAPU8.set(bytes, ptr);
    try {
      let evaluatorIndex = this.withCString(name, (namePtr) => this.moduleRef._routing_add_matrix_dimension(
        this.handle,
        ptr,
        valueArray.length,
        matrix.length,
  ...
  AddDisjunction(indices, penalty) {
    ...
    let valueArray = toInt64Array(indices), bytes = new Uint8Array(valueArray.buffer, valueArray.byteOffset, valueArray.byteLength), ptr = this.module._malloc(bytes.byteLength);
    this.module.HEAPU8.set(bytes, ptr);
    try {
      let disjunctionIndex = this.module._routing_add_disjunction(
        this.handle,
        ptr,
        valueArray.length,
        toInt64(penalty ?? 0),
  ...
  GetNumberOfDecisionsInFirstSolution(parameters) {
    ...
    let decisions = toNumber(this.module._routing_get_number_of_decisions_in_first_solution(
      this.handle,
      parameters.firstSolutionStrategy ?? 0,
      parameters.solution_limit ?? 0
    ));
  ...
  async SolveFromAssignmentWithParameters(assignment, parameters) {
    ...
    let result = new Assignment(this);
  ...
  ReadAssignmentFromRoutes(routes, ignoreInactiveIndices) {
    if (!this.module) {
      let result = this.workerResultFromRoutes(routes, ignoreInactiveIndices);
    ...
    let lengths = routes.map((route) => route.length), flat = routes.flat(), values = toInt64Array(flat), valueBytes = new Uint8Array(values.buffer, values.byteOffset, values.byteLength), lengthsBytes = toInt32Bytes(lengths), valuesPtr = this.module._malloc(valueBytes.byteLength), lengthsPtr = this.module._malloc(lengthsBytes.byteLength);
  ...
  GetAutomaticFirstSolutionStrategy() {
    ...
    let strategy = this.module._routing_get_automatic_first_solution_strategy(this.handle);
  ...
  GetArcCostForVehicle(fromIndex, toIndex, vehicle) {
    if (this.lastWorkerResult) {
      let dimension = this.manager.GetNumberOfIndices(), matrix = this.buildTransitMatrix();
  ...
  delete() {
    for (let callbackId of this.callbackIds)
  ...
  callbackForEvaluatorIndex(evaluatorIndex) {
    let callback = this.evaluatorCallbacks.get(evaluatorIndex);
    if (!callback)
      throw new Error(`RoutingModel: evaluator ${evaluatorIndex} is unavailable.`);
    return callback;
  }
  buildTransitMatrix() {
    let callback = this.callbackForEvaluator();
  ...
  buildTransitMatrixFromCallback(callback) {
    let dimension = this.manager.GetNumberOfIndices(), matrix = new BigInt64Array(dimension * dimension);
  ...
  workerResultFromRoutes(routes, ignoreInactiveIndices) {
    let dimension = this.manager.GetNumberOfIndices(), nextValues = Array.from({ length: dimension }, (_, index) => index), starts = Array.from({ length: this.manager.numVehicles }, (_, vehicle) => this.manager.GetStartIndex(vehicle)), ends = Array.from({ length: this.manager.numVehicles }, (_, vehicle) => this.manager.GetEndIndex(vehicle)), matrix = this.buildTransitMatrix(), assigned = /* @__PURE__ */ new Set(), objectiveValue = 0, arcCost = (from, to) => Number(matrix[from * dimension + to]), checkIndex = (index, label) => {
    ...
    for (let vehicle = 0; vehicle < starts.length; vehicle++) {
      let route = routes[vehicle] ?? [], previous = starts[vehicle];
      for (let [position, index] of route.entries())
    ...
    if (!ignoreInactiveIndices)
      for (let index = 0; index < dimension; index++) {
        ...
        let node = this.manager.IndexToNode(index);
  ...
  withCString(value, fn) {
    if (!this.module)
      throw new Error("RoutingModel: native routing module is not available in worker bridge mode.");
    let bytes = stringBytes(value), ptr = this.module._malloc(bytes.byteLength);
    this.module.HEAPU8.set(bytes, ptr);
    try {
      return fn(ptr);
    } finally {
  ...
  installMatrixEvaluator() {
    if (!this.module)
      return;
    let matrix = this.buildTransitMatrix(), matrixBytes = new Uint8Array(matrix.buffer, matrix.byteOffset, matrix.byteLength), matrixPtr = this.module._malloc(matrixBytes.byteLength);
    this.module.HEAPU8.set(matrixBytes, matrixPtr);
    try {
      let evaluatorIndex = this.module._routing_register_matrix_transit_callback(
        this.handle,
        matrixPtr,
        matrix.length,
  ...
  runAtSolutionCallbacks() {
    for (let callback of this.atSolutionCallbacks)
      callback();
  }
};

// ../javascript/lib/routing.ts
import {
