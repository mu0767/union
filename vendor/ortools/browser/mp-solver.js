...
async function readNativeBytes(module, fn) {
  let lenPtr = module._malloc(4), responsePtr = 0;
  try {
...
var linearSolverSchemasPromise = null, linearSolverRootPromise = null, mpModelTypePromise = null, mpModelRequestTypePromise = null, mpSolutionResponseTypePromise = null;
async function getLinearSolverSchemas() {
  return linearSolverSchemasPromise ?? (linearSolverSchemasPromise = (async () => {
    ...
    let module = await loadMpSolverModule();
    return {
...
async function resolveLinearSolverRoot() {
  return linearSolverRootPromise ?? (linearSolverRootPromise = (async () => {
    let schemas = await getLinearSolverSchemas(), optionalRoot = protobufModule.parse(schemas.optional_boolean).root, linearSolverSource = schemas.linear_solver.replace(/^import "ortools\/util\/optional_boolean\.proto";\s*$/m, "");
...
async function encodeMPModelRequest(request) {
  let type = await resolveMPModelRequestType(), error = type.verify(request);
  if (error)
...
async function encodeMPModel(model) {
  let type = await resolveMPModelType(), error = type.verify(model);
  if (error)
...
async function decodeMPSolutionResponse(bytes) {
  let type = await resolveMPSolutionResponseType();
...
async function encodeMPSolutionResponse(response) {
  let type = await resolveMPSolutionResponseType(), error = type.verify(response);
  if (error)
...
function normalizedNumThreads(options = {}) {
  let numThreads = options.numThreads ?? options.num_threads;
...
async function solveModelRequestDirect(requestBytes, options = {}) {
  let module = await loadMpSolverModule(), requestPtr = copyBytesToHeap(module, requestBytes);
  try {
    let numThreads = normalizedNumThreads(options);
...
async function solveModelRequestBytes(requestBytes, options = {}) {
  if (shouldUseMPSolverBridge()) {
    let numThreads = normalizedNumThreads(options), response = await postWorkerRequest({
      type: "mpSolverSolve",
      id: nextWorkerBridgeRequestId(),
      requestBytes,
      numThreads
...
function workerStatusHasSolution(status) {
  let resultStatus = workerStatusToResultStatus(status);
  return resultStatus === 0 /* OPTIMAL */ || resultStatus === 1 /* FEASIBLE */;
}
function workerConstraintActivity(state, constraint) {
  let value = 0;
  for (let [variableIndex, coefficient] of constraint.coeffs)
...
function workerObjectiveValue(state) {
  ...
  for (let [variableIndex, coefficient] of state.objective.coeffs)
    value += coefficient * (state.variables[variableIndex]?.solutionValue ?? 0);
  return value;
}
...
function workerModelRequest(state, options = {}) {
  ...
  let solverSpecificParameters = options.solverSpecificParameters ?? state.solverSpecificParameters;
...
function applyWorkerSolutionResponse(state, response) {
  let loaded = workerStatusHasSolution(response.status), variableValues = Array.isArray(response.variableValue) ? response.variableValue : [];
  for (let variable of state.variables) {
    let value = Number(variableValues[variable.index] ?? 0);
  ...
  let reducedCosts = Array.isArray(response.reducedCost) ? response.reducedCost : [];
  for (let variable of state.variables)
    variable.index < reducedCosts.length && (variable.reducedCost = Number(reducedCosts[variable.index]));
  let dualValues = Array.isArray(response.dualValue) ? response.dualValue : [];
  for (let constraint of state.constraints)
  ...
  let solveInfo = response.solveInfo;
...
var NativeMPSolverParametersBackend = class {
...
}, BridgeMPSolverParametersBackend = class {
  constructor() {
...
}, NativeMPSolverBackend = class _NativeMPSolverBackend {
  ...
  static createSolver(solverId) {
    let module = getMpSolverModule(), exports = getMpSolverExports(), handle = withCString(module, solverId, (solverIdPtr) => exports.solverCreateSolver(solverIdPtr));
  ...
  variable(index) {
    let handle = this.exports.solverVariable(this.handle, index);
  ...
  lookupVariable(name) {
    let handle = withCString(this.module, name, (namePtr) => this.exports.solverLookupVariable(this.handle, namePtr));
    return handle === 0 ? null : { index: this.exports.variableIndex(handle), handle };
  }
  addVariable(lb, ub, integer, name) {
    let handle = withCString(this.module, name, (namePtr) => this.exports.solverVar(this.handle, lb, ub, integer ? 1 : 0, namePtr));
  ...
  constraint(index) {
    let handle = this.exports.solverConstraint(this.handle, index);
  ...
  lookupConstraint(name) {
    let handle = withCString(this.module, name, (namePtr) => this.exports.solverLookupConstraint(this.handle, namePtr));
    return handle === 0 ? null : { index: this.exports.constraintIndex(handle), handle };
  }
  addConstraint(lb, ub, name) {
    let hasBounds = typeof lb == "number" && typeof ub == "number", handle = withCString(this.module, name, (namePtr) => hasBounds ? this.exports.solverRowConstraint(this.handle, lb, ub, namePtr) : this.exports.solverUnboundedRowConstraint(this.handle, namePtr));
  ...
  exportModelRequestProto(options = {}) {
    let solverType = options.solverType ?? this.problemType(), parameters = options.solverSpecificParameters ?? "";
  ...
  async loadSolutionFromProto(response, tolerance) {
    let responseBytes = response instanceof Uint8Array ? response : await encodeMPSolutionResponse(response), responsePtr = copyBytesToHeap(this.module, responseBytes);
    try {
  ...
  setHint(variables, values) {
    let variableBytes = variables.length * Int32Array.BYTES_PER_ELEMENT, valueBytes = values.length * Float64Array.BYTES_PER_ELEMENT, variablePtr = this.module._malloc(variableBytes), valuePtr = this.module._malloc(valueBytes);
    try {
  ...
  lookupVariable(name) {
    let variable = this.state.variables.find((candidate) => candidate.name === name);
  ...
  setVariableBounds(ref, lb, ub) {
    let variable = this.variableState(ref);
  ...
  lookupConstraint(name) {
    let constraint = this.state.constraints.find((candidate) => candidate.name === name);
  ...
  setConstraintCoefficient(constraintRef, variableRef, coefficient) {
    let constraint = this.constraintState(constraintRef);
  ...
  setConstraintBounds(ref, lb, ub) {
    let constraint = this.constraintState(ref);
  ...
  async solve() {
    let started = Date.now(), result = await MPSolver.solveModelRequest(
  ...
  async loadSolutionFromProto(response, _tolerance) {
    let decoded = response instanceof Uint8Array ? await decodeMPSolutionResponse(response) : response;
  ...
  verifySolution(tolerance) {
    ...
    for (let constraint of this.state.constraints) {
      let activity = workerConstraintActivity(this.state, constraint);
...
var _MPSolver = class _MPSolver {
  ...
  static CreateSolver(solverId) {
    if (shouldUseMPSolverBridge()) {
      let problemType = workerParseSolverType(solverId);
      return problemType !== null && workerSupportsProblemType(problemType) ? new _MPSolver(solverId, problemType) : null;
    }
    let backend = NativeMPSolverBackend.createSolver(solverId);
    if (!backend) return null;
    let solver = Object.create(_MPSolver.prototype);
  ...
  static ParseSolverType(solverId) {
    ...
    let module = getMpSolverModule(), exports = getMpSolverExports(), problemType = withCString(module, solverId, (solverIdPtr) => exports.solverParseSolverType(solverIdPtr));
    return problemType < 0 ? null : problemType;
  }
  static ParseAndCheckSupportForProblemType(solverId) {
    let problemType = _MPSolver.ParseSolverType(solverId);
  ...
  static async solveModelRequest(request, options = {}) {
    let requestBytes = request instanceof Uint8Array ? request : await encodeMPModelRequest(request), bytes = await solveModelRequestBytes(requestBytes, options);
    return {
      bytes,
  ...
  LookupVariableOrNull(name) {
    let ref = this.backend.lookupVariable(name);
  ...
  LookupConstraintOrNull(name) {
    let ref = this.backend.lookupConstraint(name);
  ...
  Constraint(lbOrName, ub, name = "") {
    let hasBounds = typeof lbOrName == "number" && typeof ub == "number", constraintName = typeof lbOrName == "string" ? lbOrName : name;
  ...
  async SolveWithProto(options = {}) {
    let requestBytes = await this.exportModelRequestProto(options), result = await _MPSolver.solveModelRequest(requestBytes, options), loaded = !1;
