...
var pdlpModulePromise = null, pdlpModule = null, terminationReasonNames = {
...
async function runWithBytes(bytes, fn) {
  let module = getPdlpModule(), ptr = copyBytesToHeap(module, bytes);
  try {
...
var BinaryWriter = class {
  ...
  doubles(values) {
    ...
    for (let value of values) this.double(value);
  }
  strings(values) {
    this.u32(values.length);
    for (let value of values) this.string(value);
  }
  finish() {
    let size = this.parts.reduce((sum, part) => sum + part.length, 0), output = new Uint8Array(size), offset = 0;
  ...
  u32() {
    let value = new DataView(this.bytes.buffer, this.bytes.byteOffset + this.offset, 4).getUint32(0, !0);
    return this.offset += 4, value;
  }
  double() {
    let value = new DataView(this.bytes.buffer, this.bytes.byteOffset + this.offset, 8).getFloat64(0, !0);
    return this.offset += 8, value;
  }
  string() {
    let size = this.u32(), value = new TextDecoder().decode(this.bytes.slice(this.offset, this.offset + size));
...
function encodeQuadraticProgram(input) {
  let qp = normalizeQuadraticProgram(input), writer = new BinaryWriter();
...
function encodeParams(params = {}) {
  let writer = new BinaryWriter(), terminationCriteria = params.terminationCriteria ?? params.termination_criteria, simple = terminationCriteria?.simpleOptimalityCriteria ?? terminationCriteria?.simple_optimality_criteria, iterationLimit = terminationCriteria?.iterationLimit ?? terminationCriteria?.iteration_limit;
  iterationLimit !== void 0 ? (writer.u8(1), writer.u32(iterationLimit)) : writer.u8(0);
  let terminationCheckFrequency = params.terminationCheckFrequency ?? params.termination_check_frequency;
  terminationCheckFrequency !== void 0 ? (writer.u8(1), writer.u32(terminationCheckFrequency)) : writer.u8(0);
  let epsOptimalRelative = simple?.epsOptimalRelative ?? simple?.eps_optimal_relative;
  epsOptimalRelative !== void 0 ? (writer.u8(1), writer.double(epsOptimalRelative)) : writer.u8(0);
  let epsOptimalAbsolute = simple?.epsOptimalAbsolute ?? simple?.eps_optimal_absolute;
  epsOptimalAbsolute !== void 0 ? (writer.u8(1), writer.double(epsOptimalAbsolute)) : writer.u8(0);
  let lInfRuizIterations = params.lInfRuizIterations ?? params.l_inf_ruiz_iterations;
  lInfRuizIterations !== void 0 ? (writer.u8(1), writer.u32(lInfRuizIterations)) : writer.u8(0);
  let l2NormRescaling = params.l2NormRescaling ?? params.l2_norm_rescaling;
...
function decodeSolverResult(bytes) {
  ...
  let primalSolution = reader.doubles(), dualSolution = reader.doubles(), reducedCosts = reader.doubles(), terminationReasonNumber = reader.u32(), iterationCount = reader.u32(), solveLog = {
...
var QuadraticProgram = class {
  ...
  assign(input) {
    let qp = normalizeQuadraticProgram(input);
  ...
  async validateQuadraticProgramDimensions(qp) {
    let message = new TextDecoder().decode(await pdlpBytes("validate", encodeQuadraticProgram(qp)));
  ...
  async qpFromMpModelProto(proto, options = {}) {
    let bytes = await pdlpBytes("fromMpModel", proto, options);
  ...
  async qpToMpModelProto(qp) {
    let bytes = await pdlpBytes("toMpModel", encodeQuadraticProgram(qp));
  ...
  async primalDualHybridGradient(qp, params = {}, initialSolution) {
    let bytes = concat([encodeQuadraticProgram(qp), encodeParams(params), encodeInitialSolution(initialSolution)]), resultBytes = await pdlpBytes("solve", bytes);
...
function concat(parts) {
  let size = parts.reduce((sum, part) => sum + part.length, 0), output = new Uint8Array(size), offset = 0;
  for (let part of parts)
    output.set(part, offset), offset += part.length;
  return output;
}

// ../javascript/lib/pdlp.ts
import {
