...
var ConsistencyLevel = /* @__PURE__ */ ((ConsistencyLevel2) => (ConsistencyLevel2[ConsistencyLevel2.COST_AND_COVERAGE = 1] = "COST_AND_COVERAGE", ConsistencyLevel2[ConsistencyLevel2.FREE_AND_UNCOVERED = 2] = "FREE_AND_UNCOVERED", ConsistencyLevel2[ConsistencyLevel2.REDUNDANCY = 3] = "REDUNDANCY", ConsistencyLevel2))(ConsistencyLevel || {}), consistency_level = ConsistencyLevel, operationCode = {
  trivial: 0,
  greedy: 1,
  elementDegree: 2,
  lazyElementDegree: 3,
  ...
  guidedLocal: 6,
  guidedTabu: 7
}, setCoverModulePromise = null, setCoverModule = null;
...
function stats(values) {
  ...
  let sorted = [...values].sort((a, b) => a - b), mean = sorted.reduce((sum, value) => sum + value, 0) / sorted.length, variance = sorted.reduce((sum, value) => sum + (value - mean) ** 2, 0) / sorted.length;
...
var SetCoverModelStats = class {
  ...
  get num_elements() {
    let max = -1;
    for (let subset of this.subsetElements)
      for (let element of subset) max = Math.max(max, element);
    return max + 1;
  }
  get num_subsets() {
  ...
  get fill_rate() {
    let denominator = this.num_elements * this.num_subsets;
  ...
  compute_feasibility() {
    ...
    for (let subset of this.subsetElements)
      for (let element of subset) covered.add(element);
  ...
  compute_coverage_in_focus(focus) {
    let coverage = Array.from({ length: this.currentModel.num_elements }, () => 0), columns = this.currentModel.columns;
    for (let subset of focus) {
      assertSubsetIndex(subset, this.currentModel.num_subsets);
      for (let element of columns[subset]) coverage[element]++;
    }
    return coverage;
  }
  is_redundant() {
  ...
  recompute() {
    let columns = this.currentModel.columns, costs = this.currentModel.subset_costs;
    this.currentCoverage = Array.from({ length: this.currentModel.num_elements }, () => 0), this.currentCost = 0, this.selected.forEach((value, subset) => {
      if (value) {
        ...
        for (let element of columns[subset] ?? []) this.currentCoverage[element]++;
      }
  ...
  export_solution_as_proto() {
    let subsets = this.selected.flatMap((value, subset) => value ? [subset] : []);
    return createSolutionResponse(subsets, this.currentCost, this.currentModel.num_subsets);
  }
  import_solution_from_proto(proto) {
    let selected = Array.from({ length: this.currentModel.num_subsets }, () => !1);
    for (let subset of proto.subset ?? [])
...
}, SetCoverSolutionGenerator = class {
  ...
  async next_solution(focus) {
    let model = this.invariant.model(), focusMask = null;
    ...
    let result = await runNativeSetCover(model._nativePayload(
...
}, TrivialSolutionGenerator = class extends SetCoverSolutionGenerator {
  constructor(invariant) {
    super(invariant, "trivial", "TrivialGenerator");
  }
}, RandomSolutionGenerator = class extends SetCoverSolutionGenerator {
  constructor(invariant) {
    super(invariant, "random", "RandomGenerator");
  }
}, GreedySolutionGenerator = class extends SetCoverSolutionGenerator {
  constructor(invariant) {
    super(invariant, "greedy", "GreedyGenerator");
  }
}, ElementDegreeSolutionGenerator = class extends SetCoverSolutionGenerator {
  constructor(invariant) {
    super(invariant, "elementDegree", "ElementDegreeGenerator");
  }
}, LazyElementDegreeSolutionGenerator = class extends SetCoverSolutionGenerator {
  constructor(invariant) {
    super(invariant, "lazyElementDegree", "LazyElementDegreeGenerator");
  }
}, SteepestSearch = class extends SetCoverSolutionGenerator {
  constructor(invariant) {
    super(invariant, "steepest", "SteepestSearch");
  }
}, GuidedLocalSearch = class extends SetCoverSolutionGenerator {
  constructor(invariant) {
...
}, GuidedTabuSearch = class extends SetCoverSolutionGenerator {
  constructor(invariant) {
...
function clear_random_subsets(numSubsetsOrFocus, invariantOrNumSubsets, maybeInvariant) {
  let invariant = typeof numSubsetsOrFocus == "number" ? invariantOrNumSubsets : maybeInvariant, numSubsets = typeof numSubsetsOrFocus == "number" ? numSubsetsOrFocus : invariantOrNumSubsets;
  if (!invariant) throw new Error("clear_random_subsets: invariant is required.");
  let selected = invariant.is_selected(), chosen = [];
