...
var MathOptVarEqVar = class {
...
}, MathOptSparseVectorFilter = class {
  constructor(options = {}) {
    __publicField(this, "options", options);
  }
  toProtoBytes() {
...
}, MathOptSolutionHint = class {
  constructor(options = {}) {
    __publicField(this, "options", options);
  }
  toProtoBytes() {
    let variableValues = this.options.variableValues ?? this.options.variable_values, dualValues = this.options.dualValues ?? this.options.dual_values;
    return message([
      ...
      fieldMessageIfPresent(2, dualValues === void 0 ? null : encodeLinearConstraintDoubleVector(dualValues.map((entry) => {
        let normalized = entry, linearConstraint = normalized.linearConstraint ?? normalized.linear_constraint;
  ...
  static onlySomePrimalVariables(variables) {
    return new _MathOptModelSolveParameters({
      variableValuesFilter: { elements: variables, filterByIds: !0 }
    });
  }
  static only_some_primal_variables(variables) {
    return _MathOptModelSolveParameters.onlySomePrimalVariables(variables);
  }
}, MathOptQuadraticTermKey = class {
  constructor(firstVariable, secondVariable) {
    __publicField(this, "firstVariable");
    __publicField(this, "secondVariable");
    if (firstVariable.model !== secondVariable.model)
...
}, MathOptLinearExpression = class _MathOptLinearExpression {
  constructor(terms = [], offset = 0) {
    ...
    if (terms instanceof _MathOptLinearExpression || terms instanceof MathOptVariable || isLinearTerm(terms)) {
      let expression = asFlatLinearExpression(terms);
    ...
    for (let term of terms) {
      ...
      let existing = findVariableKey(merged, term.variable), next = (existing ? merged.get(existing) ?? 0 : 0) + term.coefficient;
      existing && merged.delete(existing), next !== 0 && merged.set(term.variable, next);
    }
    this.terms = readonlyMap(merged), this.offset = offset;
  }
  add(input) {
    let rhs = asFlatLinearExpression(input);
    return new _MathOptLinearExpression([
...
}, MathOptQuadraticExpression = class _MathOptQuadraticExpression {
  constructor(linearTerms = [], quadraticTerms = [], offset = 0) {
    __publicField(this, "offset");
    __publicField(this, "linearTerms");
    __publicField(this, "quadraticTerms");
    if (typeof linearTerms == "number" || linearTerms instanceof MathOptVariable || linearTerms instanceof MathOptLinearExpression || linearTerms instanceof _MathOptQuadraticExpression || isLinearTerm(linearTerms) || isQuadraticTerm(linearTerms)) {
      let expression = asFlatQuadraticExpression(linearTerms);
    ...
    let merged = /* @__PURE__ */ new Map();
    for (let term of quadraticTerms) {
      if (!isQuadraticTerm(term))
        throw new TypeError("unsupported type in iterable argument");
      let key = new MathOptQuadraticTermKey(term.firstVariable, term.secondVariable), existing = findQuadraticKey(merged, key), next = (existing ? merged.get(existing) ?? 0 : 0) + term.coefficient;
      existing && merged.delete(existing), next !== 0 && merged.set(key, next);
    }
    this.quadraticTerms = readonlyMap(merged), this.offset = offset;
  }
  add(input) {
    let rhs = asFlatQuadraticExpression(input);
    return new _MathOptQuadraticExpression(
...
}, MathOptBoundedExpression = class {
...
}, MathOptLowerBoundedExpression = class {
...
}, MathOptUpperBoundedExpression = class {
...
var MathOptModel = class {
  ...
  addIndicatorConstraint(options = {}) {
    let id = this.indicatorConstraints.length, normalizedOptions = this.normalizeIndicatorConstraintOptions(options);
    normalizedOptions.indicator && this.assertOwnsVariable(normalizedOptions.indicator);
    let expression = normalizedOptions.expression === void 0 ? new MathOptLinearExpression(normalizedOptions.terms ?? []) : asFlatLinearExpression(normalizedOptions.expression).add(new MathOptLinearExpression(normalizedOptions.terms ?? []));
    if (!Number.isFinite(expression.offset))
  ...
  normalizeIndicatorConstraintOptions(options) {
    ...
    let implied = options.impliedConstraint ?? options.implied_constraint;
  ...
  deleteVariable(variable) {
    ...
    for (let constraint of this.indicatorConstraints)
  ...
  ensureNextVariableIdAtLeast(id) {
    for (; this.variableData.length < id; ) {
      let placeholderId = this.variableData.length;
  ...
  getVariable(id, validate = !0) {
    let variable = this.variableData[id];
  ...
  get_variable(id, options) {
    let variable = this.getVariable(id, options?.validate ?? !0);
  ...
  ensureNextLinearConstraintIdAtLeast(id) {
    for (; this.constraints.length < id; ) {
      let placeholderId = this.constraints.length;
  ...
  getLinearConstraint(id, validate = !0) {
    let constraint = this.constraints[id];
  ...
  get_linear_constraint(id, options) {
    let constraint = this.getLinearConstraint(id, options?.validate ?? !0);
  ...
  linearConstraintMatrixEntries() {
    return this.constraints.filter((constraint) => !constraint.deleted).flatMap((constraint) => {
      let linearConstraint = new MathOptLinearConstraint(this, constraint);
  ...
  encodeModelProto(options = {}) {
    let removeNames = options.removeNames ?? !1;
    return message([
  ...
  encodeVariables(options = {}) {
    let activeVariables = this.variableData.filter((variable) => !variable.deleted);
    return message([
  ...
  encodeObjective() {
    return message([
      ...
      fieldMessage(4, encodeSparseDoubleMatrix(this.objectiveDataValue.quadraticTerms.map((term) => {
        let rowId = Math.min(term.firstVariable.id, term.secondVariable.id), columnId = Math.max(term.firstVariable.id, term.secondVariable.id);
        return { rowId, columnId, coefficient: term.coefficient };
      })))
    ]);
  }
  encodeLinearConstraints(options = {}) {
    let activeConstraints = this.constraints.filter((constraint) => !constraint.deleted);
    return message([
  ...
  encodeLinearConstraintMatrix() {
    let entries = this.constraints.filter((constraint) => !constraint.deleted).flatMap((constraint) => [...constraint.terms].filter((term) => term.coefficient !== 0).sort((a, b) => a.variable.id - b.variable.id).map((term) => ({
      rowId: constraint.id,
  ...
  setCoefficient(variable, coefficient) {
    ...
    let existingIndex = this.data.terms.findIndex((term2) => term2.variable.id === variable.id);
    if (coefficient === 0) {
...
}, MathOptIndicatorConstraint = class {
  constructor(model, data) {
    __publicField(this, "model", model);
    __publicField(this, "data", data);
  }
  ...
  setLinearCoefficient(variable, coefficient) {
    ...
    let terms = this.model.objectiveData.linearTerms.filter((term) => term.variable.id !== variable.id);
  ...
  setQuadraticCoefficient(firstVariable, secondVariable, coefficient) {
    ...
    let key = new MathOptQuadraticTermKey(firstVariable, secondVariable), terms = this.model.objectiveData.quadraticTerms.filter((term) => !new MathOptQuadraticTermKey(term.firstVariable, term.secondVariable).equals(key));
  ...
  getQuadraticCoefficient(firstVariable, secondVariable) {
    this.model.assertOwnsVariable(firstVariable), this.model.assertOwnsVariable(secondVariable);
    let key = new MathOptQuadraticTermKey(firstVariable, secondVariable);
...
function formatExpression(offset, linearTerms, quadraticTerms) {
  ...
  for (let term of [...linearTerms].filter((term2) => term2.coefficient !== 0).sort((lhs, rhs) => compareVariables(lhs.variable, rhs.variable)))
    result += formatSignedTerm(term.coefficient, term.variable.toString());
  for (let term of [...quadraticTerms].filter((term2) => term2.coefficient !== 0).sort((lhs, rhs) => compareVariables(lhs.firstVariable, rhs.firstVariable) || compareVariables(lhs.secondVariable, rhs.secondVariable)))
...
function asFlatQuadraticExpression(input) {
  ...
  let linear = asFlatLinearExpression(input);
...
function multiplyLinearExpressions(lhs, rhs) {
  let lhsFlat = asFlatLinearExpression(lhs), rhsFlat = asFlatLinearExpression(rhs), linearTerms = [], quadraticTerms = [];
  for (let term of linearTermEntries(lhsFlat)) {
    rhsFlat.offset !== 0 && linearTerms.push({
      variable: term.variable,
      coefficient: term.coefficient * rhsFlat.offset
    });
    for (let rhsTerm of linearTermEntries(rhsFlat))
      quadraticTerms.push({
  ...
  if (lhsFlat.offset !== 0)
    for (let term of linearTermEntries(rhsFlat))
      linearTerms.push({
        variable: term.variable,
...
function evaluateExpression(expression, variableValues) {
  let values = (variable) => {
    if (variableValues instanceof Map) {
      let typedValues = variableValues, matchingVariable = findVariableKey(typedValues, variable);
      return typedValues.get(variable) ?? (matchingVariable ? typedValues.get(matchingVariable) : void 0) ?? 0;
    }
    let record = variableValues;
    return record[variable.id] ?? record[variable.name] ?? 0;
  }, flat = asFlatQuadraticExpression(expression), result = flat.offset;
  for (let [variable, coefficient] of flat.linearTerms)
    result += coefficient * values(variable);
  for (let [key, coefficient] of flat.quadraticTerms)
...
function eq(lhs, rhs) {
  ...
  let expression = isQuadraticOnlyInput(lhs) || isQuadraticOnlyInput(rhs) ? isQuadraticOnlyInput(lhs) ? asFlatQuadraticExpression(lhs).subtract(rhs) : asFlatQuadraticExpression(rhs).subtract(lhs) : asFlatLinearExpression(lhs).subtract(rhs);
...
var MathOptIncrementalSolver = class {
  ...
  async create() {
    let requestBytes = MathOpt.encodeSolveRequest(this.model, {
      ...this.options,
    ...
    if (statusBytes) {
      let status = readMessage(statusBytes);
      throw new Error(status.strings.get(2)?.[0] ?? "MathOpt incremental solver creation failed.");
    }
    let handleText = response.strings.get(2)?.[0], handle = handleText === void 0 ? 0 : Number(handleText);
  ...
  async solve(options = {}) {
    ...
    let handle = await this.initPromise, mergedOptions = {
      ...this.options,
      ...options,
      solverType: this.solverType
    ...
    let messageCallback = solveMessageCallback(mergedOptions);
  ...
  async close() {
    if (!this.closed) {
      this.closed = !0;
      try {
        let handle = this.handle ?? await this.initPromise.catch(() => 0);
...
async function solveDirect(requestBytes, interrupterState) {
  let module = await loadMathOptModule(), requestPtr = copyBytesToHeap(module, requestBytes), lenPtr = module._malloc(4);
  try {
    let ptr = await module.ccall(
...
async function incrementalSolveDirect(handle, requestBytes, updateBytes, interrupterState) {
  let module = await loadMathOptModule(), requestPtr = copyBytesToHeap(module, requestBytes), updatePtr = updateBytes ? copyBytesToHeap(module, updateBytes) : 0, lenPtr = module._malloc(4);
  try {
    let ptr = await module.ccall(
...
function encodeSolveRequest(model, options) {
  let solverType = typeof options.solverType == "string" ? MathOptSolverType[options.solverType] : options.solverType ?? 3 /* GLOP */;
  ...
  let parameters = encodeMathOptSolveParameters(options), modelParameters = modelParametersBytes(options.modelParameters ?? options.model_parameters), removeNames = options.removeNames ?? options.remove_names ?? !1;
  return message([
...
function encodeMathOptSolveParameters(options) {
  let raw = options.parameters ?? options.solveParameters ?? options.solve_parameters;
  if (raw) return solveParametersBytes(raw);
  let enableOutput = options.enableOutput ?? options.enable_output ?? (solveMessageCallback(options) ? !0 : void 0), fields = [
...
function objectiveData(maximize, terms, offset) {
  let expression = Array.isArray(terms) ? new MathOptLinearExpression(terms, offset) : asFlatQuadraticExpression(terms).add(offset);
...
function linearObjectiveData(maximize, terms, offset) {
  let expression = Array.isArray(terms) ? new MathOptLinearExpression(terms, offset) : asFlatLinearExpression(terms).add(offset);
  return {
    maximize,
    linearTerms: linearTermEntries(expression),
    quadraticTerms: [],
...
function encodeSparseDoubleVector(terms) {
  let sortedTerms = [...terms].sort((a, b) => a.variable.id - b.variable.id);
  return message([
...
function encodeLinearConstraintDoubleVector(terms) {
  let sortedTerms = [...terms].sort((a, b) => a.linearConstraint.id - b.linearConstraint.id);
  return message([
...
function encodeSparseDoubleMatrix(terms) {
  let sortedTerms = [...terms].sort((a, b) => a.rowId - b.rowId || a.columnId - b.columnId);
  return message([
...
function encodeModelUpdate(model, snapshot, options = {}) {
  let current = model.snapshot(), previousVariables = new Map(snapshot.variables.map((variable) => [variable.id, variable])), currentVariables = new Map(current.variables.map((variable) => [variable.id, variable])), previousConstraints = new Map(snapshot.linearConstraints.map((constraint) => [constraint.id, constraint])), currentConstraints = new Map(current.linearConstraints.map((constraint) => [constraint.id, constraint])), previousIndicators = new Map(snapshot.indicatorConstraints.map((constraint) => [constraint.id, constraint])), currentIndicators = new Map(current.indicatorConstraints.map((constraint) => [constraint.id, constraint])), deletedVariableIds = snapshot.variables.filter((variable) => !variable.deleted && currentVariables.get(variable.id)?.deleted).map((variable) => variable.id), deletedLinearConstraintIds = snapshot.linearConstraints.filter((constraint) => !constraint.deleted && currentConstraints.get(constraint.id)?.deleted).map((constraint) => constraint.id), deletedIndicatorConstraintIds = snapshot.indicatorConstraints.filter((constraint) => !constraint.deleted && currentIndicators.get(constraint.id)?.deleted).map((constraint) => constraint.id), newVariables = current.variables.filter((variable) => {
    ...
    let previous = previousIndicators.get(constraint.id);
    return !constraint.deleted && (!previous || previous.deleted);
...
function encodeVariablesSnapshot(variables, options = {}) {
  let active = [...variables].filter((variable) => !variable.deleted).sort((a, b) => a.id - b.id);
...
function encodeLinearConstraintsSnapshot(constraints, options = {}) {
  let active = [...constraints].filter((constraint) => !constraint.deleted).sort((a, b) => a.id - b.id);
  return active.length === 0 ? null : message([
...
function encodeSparseDoubleVectorById(terms) {
  let sortedTerms = [...terms].filter((term) => term.coefficient !== 0).sort((a, b) => a.variableId - b.variableId);
  return message([
...
function changedValues(previous, current, value) {
  let currentById = new Map(current.map((item) => [item.id, item]));
  return previous.flatMap((oldItem) => {
    if (oldItem.deleted) return [];
    let newItem = currentById.get(oldItem.id);
...
function vectorUpdate(field, updates, encodeValues) {
  ...
  let sorted = [...updates].sort((a, b) => a.id - b.id);
...
function changedMatrixEntries(previous, current) {
  let previousById = new Map(previous.map((constraint) => [constraint.id, constraint])), updates = [];
  for (let constraint of current) {
    if (constraint.deleted) continue;
    let previousConstraint = previousById.get(constraint.id), previousTerms = termsByVariableId(previousConstraint && !previousConstraint.deleted ? previousConstraint.terms : []), currentTerms = termsByVariableId(constraint.terms), variableIds = /* @__PURE__ */ new Set([...previousTerms.keys(), ...currentTerms.keys()]);
    for (let variableId of variableIds) {
      let oldCoefficient = previousTerms.get(variableId) ?? 0, newCoefficient = currentTerms.get(variableId) ?? 0;
...
function encodeObjectiveUpdate(previous, current) {
  let linearUpdates = changedTermMap(
...
function changedTermMap(previousEntries, currentEntries) {
  let previous = new Map(previousEntries), current = new Map(currentEntries), ids = /* @__PURE__ */ new Set([...previous.keys(), ...current.keys()]), updates = [];
  for (let id of ids) {
    let oldCoefficient = previous.get(id) ?? 0, newCoefficient = current.get(id) ?? 0;
...
function changedQuadraticTermMap(previousTerms, currentTerms) {
  let key = (term) => `${term.firstVariableId}:${term.secondVariableId}`, previous = new Map(previousTerms.map((term) => [key(term), term])), current = new Map(currentTerms.map((term) => [key(term), term])), keys = /* @__PURE__ */ new Set([...previous.keys(), ...current.keys()]), updates = [];
  for (let termKey of keys) {
    let oldTerm = previous.get(termKey), coefficient = current.get(termKey)?.coefficient ?? 0;
    if (!Object.is(oldTerm?.coefficient ?? 0, coefficient)) {
      let [firstVariableId, secondVariableId] = termKey.split(":").map(Number);
...
function decodeSolveResponse(bytes, model) {
  ...
  return {
    ...
    reduced_costs: ((input) => {
      let dualSolution = bestSolution?.dualSolution ?? null;
    ...
    variable_status: ((input) => {
      let basis = bestSolution?.basis ?? null;
      if (basis === null)
...
function decodeSolution(bytes, model) {
  let solution = readMessage(bytes), primalBytes = solution.messages.get(1)?.[0], dualBytes = solution.messages.get(2)?.[0], basisBytes = solution.messages.get(3)?.[0];
  return {
...
function decodePrimalSolution(bytes, model) {
  let primal = readMessage(bytes), variableValues = decodeSparseDoubleVector(
...
function decodeDualSolution(bytes, model) {
  let dual = readMessage(bytes), dualValues = decodeSparseDoubleVector(
...
function decodePrimalRay(bytes, model) {
  let ray = readMessage(bytes), variableValues = decodeSparseDoubleVector(
...
function decodeDualRay(bytes, model) {
  let ray = readMessage(bytes), dualValues = decodeSparseDoubleVector(
    ray.messages.get(1)?.[0],
    (id) => model.linearConstraintName(id)
  ), reducedCosts = decodeSparseDoubleVector(
...
function decodeBasis(bytes, model) {
  let basis = readMessage(bytes), constraintStatus = decodeSparseBasisStatusVector(
...
function decodeDurationSeconds(bytes) {
  let duration = readMessage(bytes), seconds = Number(duration.varints.get(1)?.[0] ?? 0n), nanos = Number(duration.varints.get(2)?.[0] ?? 0n);
  return seconds + nanos / 1e9;
}
function decodeSparseBasisStatusVector(bytes, nameForId) {
  let byId = {}, byName = {};
  if (!bytes) return { byId, byName };
  let sparse = readMessage(bytes), ids = sparse.packedVarints.get(1) ?? [], values = sparse.packedVarints.get(2) ?? [];
  return ids.forEach((id, index) => {
    let numericId = Number(id), statusNumber = Number(values[index] ?? 0n), status = basisStatusNames[statusNumber] ?? `BASIS_STATUS_${statusNumber}`;
...
function decodeSparseDoubleVector(bytes, nameForId) {
  let byId = {}, byName = {};
  if (!bytes) return { byId, byName };
  let sparse = readMessage(bytes), ids = sparse.packedVarints.get(1) ?? [], values = sparse.packedDoubles.get(2) ?? [];
  return ids.forEach((id, index) => {
    let numericId = Number(id), value = values[index] ?? 0;
...
function readMessage(bytes) {
  ...
  for (; offset < bytes.length; ) {
    let key = readVarint(bytes, offset);
    offset = key.offset;
    let field = Number(key.value >> 3n), wire = Number(key.value & 7n);
    if (wire === 0) {
      let value = readVarint(bytes, offset);
      offset = value.offset, pushMap(decoded.varints, field, value.value);
    } else if (wire === 1) {
      let value = new DataView(bytes.buffer, bytes.byteOffset + offset, 8).getFloat64(0, !0);
      offset += 8, pushMap(decoded.doubles, field, value);
    } else if (wire === 2) {
      let length = readVarint(bytes, offset);
      offset = length.offset;
      let end = offset + Number(length.value), payload = bytes.slice(offset, end);
...
function readPackedDoubles(bytes) {
  let values = [], view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
...
function fieldPackedDoubles(field, values) {
  let bytes = new Uint8Array(values.length * 8), view = new DataView(bytes.buffer);
...
function fieldDurationSeconds(field, seconds) {
  ...
  let wholeSeconds = Math.trunc(seconds), nanos = Math.round((seconds - wholeSeconds) * 1e9);
  return fieldMessage(field, message([
...
function solveInterrupterState(options) {
  let interrupter = options.interrupter ?? options.solveInterrupter ?? options.solve_interrupter;
...
function encodeSparseVectorFilter(options) {
  let explicitIds = options.ids ?? options.filteredIds ?? options.filtered_ids, elementIds = options.elements?.map((element) => {
...
function encodePdlpTerminationCriteria(criteria) {
  ...
  let simple = criteria.simpleOptimalityCriteria ?? criteria.simple_optimality_criteria, encoded = message([
...
function concat(parts) {
  let length = parts.reduce((sum, part) => sum + part.length, 0), output = new Uint8Array(length), offset = 0;
